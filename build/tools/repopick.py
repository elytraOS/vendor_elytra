...
if __name__ == '__main__':
    # Default to elytra Gerrit
    default_gerrit = ''

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from elytra's Gerrit instance (or any gerrit instance of your choosing)
    ...
    for project in projects:
        name = ("elytraOS")+project.get('name')
    ...
    for item in mergables:
        ...
        # Try fetching from elytra Org first if using default gerrit
        if args.gerrit == default_gerrit:
            if args.verbose:
                print('Trying to fetch the change from elytraOS Org')

            if args.pull:
                cmd = ['git pull --no-edit elytra', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch elytra', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or elytra org failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
                    print('Fetching from elytra Org didn\'t work, trying to fetch the change from Gerrit')
                else:
