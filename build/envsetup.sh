function __print_elytraos_functions_help() {
cat <<EOF
Additional elytraOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- elytragerrit:   A Git wrapper that fetches/pushes patch from/to elytraOS Gerrit Review.
- elytrarebase:   Rebase a Gerrit change and push it again.
- elytraremote:   Add git remote for elytraOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for elytraOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the elytraOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch elytra_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/elytra-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.elytra.device | grep -q "$ELYTRA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ELYTRA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function elytraremote()
{
    ...
    git remote rm elytra 2> /dev/null
    ...
    local ELYTRA_USER=$(git config --get review.review.elytraos.org.username)
    if [ -z "$ELYTRA_USER" ]
    then
        git remote add elytra ssh://review.elytraos.org:29418/$PFX$PROJECT
    else
        git remote add elytra ssh://$ELYTRA_USER@review.elytraos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'elytra' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/elytraOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.elytra.device | grep -q "$ELYTRA_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        elytraremote
        git push elytra HEAD:refs/heads/'$1'
    fi
    '
}

function elytragerrit() {
    ...
    local user=`git config --get review.review.elytraos.org.username`
...
function elytrarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "elytraOS Gerrit Rebase Usage: "
        echo "      elytrarebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.elytraos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.elytra.device | grep -q "$ELYTRA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ELYTRA_FIXUP_COMMON_OUT ]; then
