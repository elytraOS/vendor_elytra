...
func init() {
	android.RegisterModuleType("elytra_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = elytraExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = elytraExpandVariables(ctx, dep_file)
	...
	cmd := elytraExpandVariables(ctx, String(g.properties.Cmd))
